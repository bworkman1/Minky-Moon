<div class="col-md-12">
    <div id="currentUrl" class="x_panel" data-url="<?php echo current_url(); ?>">
        <div class="x_title">
            <h2><i class="fa fa-shopping-bag"></i> Fabrics for <?php echo $product->name; ?></h2>
            <ul class="nav navbar-right panel_toolbox">
                <li>
                    <select id="fabricByProduct" class="form-control">
                        <?php
                            echo '<option value="">View All Fabrics </option>';
                            foreach($products as $pro) {
                                if($pro->id == $this->uri->segment(3)) {
                                    echo '<option selected value="'.$pro->id.'">'.$pro->name.'</option>';
                                } else {
        ...
        <div class="x_content">
                ...
                <?php
                    ...
                    $productFabricIds = array();
                    $nonProductFabrics = array();
                    if($fabric) {
                                ...
                                echo '<div class="panel-body">';
                                    echo '<ul class="list-unstyled">';
                                        ...
                                        foreach ($category as $fabrics) {
                                            if($product->id != $fabrics->product_id) {
                                                $nonProductFabrics[$key][] = $fabrics;
                                                ...
                                                $productFabricIds[] = $fabrics->id;
                                            }
                                        }

                                        if($added == false) {
                                            echo '<li><div class="alert alert-warning"><i class="fa fa-exclamation-triangle"></i> No Fabrics found in this category</div></li>';
                                        }
                                    echo '</ul>';
                                echo '</div>';
    ...
    <div class="x_panel">
        <div class="x_title">
            <h2><i class="fa fa-ban text-danger"></i> Fabrics not associated with the <?php echo $product->name; ?></h2>
            <ul class="nav navbar-right panel_toolbox">
                <li>
                    <a href="#" class="openFabricPanel"><i class="fa fa-plus"></i> Add New</a>
                </li>
        ...
        <div class="x_content">
            <?php
            if($nonProductFabrics) {
                $inView = array();
                foreach($nonProductFabrics as $key => $category) {
                    ...
                    $added = false;
                    foreach ($category as $fabrics) {
                        if(!in_array($fabrics->id, $inView) && !in_array($fabrics->id, $productFabricIds)) {
                            $inView[] = $fabrics->id;
                            $featured = '';
                            if ($fabrics->Featured == 'Yes') {
                                $featured = 'greenBorder';
